...

{
    ...
    // Run through all of our examples
    await RunExamples(podcastIndex);
}

// Calling the AddPodcastIndexSharp extension method and passing in our application's configuration
IHostBuilder CreateHostBuilder(string[] args, IConfiguration config) =>
    Host.CreateDefaultBuilder(args).ConfigureServices((_, services) => services.AddPodcastIndexSharp(config));

async Task RunExamples(IPodcastIndex podcastIndex)
{
